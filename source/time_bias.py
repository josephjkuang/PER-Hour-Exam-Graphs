...
# Data
delayed_feedback_bias = []
delayed_feedback_time = []
immediate_feeback_bias = []
...
def parse_file(): # Dividing the Data into appropriate bins
	for row in r_list:
		if row[1] != "NULL" and row[1] != "EX" and row[3] != "NULL" and row[8] != "NULL":
			if float(row[3]) > 100: # This is for typos with extra zeros
					row[3] = 100

			if int(row[5]) == 1:
				delayed_feedback_bias.append(abs(float(row[1]) - float(row[3])))
				delayed_feedback_time.append(abs(float(row[8])))
			else:
				immediate_feeback_bias.append(abs(float(row[1]) - float(row[3])))
				immediate_feedback_time.append(abs(float(row[8])))

def draw_graph(name):
	# Scatter Plot
	plt.scatter(immediate_feedback_time, immediate_feeback_bias, alpha=0.5)
	plt.title(name)
	plt.xlabel('Hours Before Exam')
	plt.ylabel('Bias')

	# Getting R and Slope
	result = scipy.stats.linregress(immediate_feedback_time, immediate_feeback_bias)
...
parse_file()
draw_graph("Immediate Feedback Bias by Time")
